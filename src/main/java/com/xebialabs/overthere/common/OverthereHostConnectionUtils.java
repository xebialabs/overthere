...
/**
 * Contains utility methods for the implementations of {@link HostConnection host connections}. Not for part of the public Overthere API.
 */
public class OverthereHostConnectionUtils {
	...
	public static LsResults getFileInfo(OverthereFile file) throws RuntimeIOException {
		...
		int errno = file.getConnection().execute(capturedOutput, "ls", "-ld", file.getPath());
		if (errno == 0) {
			results.exists = true;
	...
	private static Logger logger = LoggerFactory.getLogger(OverthereHostConnectionUtils.class);
